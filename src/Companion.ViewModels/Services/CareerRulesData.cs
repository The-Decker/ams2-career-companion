...
public sealed record CareerRulesData
{
    ...
    /// <summary>The era-skin override table (<c>data\rules\era-themes.json</c>): per-decade
    /// medium/accent/font/texture overrides for the period document + news chrome. DISPLAY-ONLY —
    /// never a fold input; empty when the file is absent (every surface then resolves the built-in
    /// <see cref="Companion.Core.Career.EraThemes"/> table).</summary>
    public required Companion.Core.Career.EraThemeCatalog EraThemes { get; init; }
    ...
    public static CareerRulesData Load(string rulesDirectory)
    {
        ...
        return new CareerRulesData
        {
            ...
            NewsDesks = Companion.Core.Newsroom.NewsDesks.Load(
                Path.Combine(rulesDirectory, "newsroom")),
            EraThemes = Companion.Core.Career.EraThemeCatalog.Load(rulesDirectory),
        };
    }
