...
5. **Race format (added 2026-07-02, desktop session):** every round defaults to **100% real race distance** — packs carry the actual historical lap counts, and the app's difficulty/OPI math assumes full-length races. Every round ships a **setup guide** in the Race Day briefing: the exact in-game session settings (laps, date/time of day, weather, opponent count) plus per-round car-setup notes where the pack author provides them.
